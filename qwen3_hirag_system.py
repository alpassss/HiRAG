"""
HiRAG System with Qwen3 8B and Hugging Face Embedding Models
This script provides a complete system for using Qwen3 8B model and embedding models from Hugging Face
in the HiRAG framework. It includes progress bars and timing for each step.
"""
import asyncio
import json
import os
...
# Import necessary HiRAG components
from hirag.hirag import HiRAG
from hirag._utils import EmbeddingFunc, wrap_embedding_func_with_attrs
...
class Qwen3HiRAGConfig:
    ...
    working_dir: str = "./hirag_cache_qwen3"
...
class Qwen3HiRAG:
    ...
    def _init_models(self):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto"  # Use available device (GPU if available)
        )
...
async def query_with_progress(hirag_instance, query_text: str, query_param: QueryParam = None):
...
def main():
    """
    Main function to run the complete HiRAG system with Qwen3 8B
    """
    print("Starting Qwen3 HiRAG System...")
    ...
    config = Qwen3HiRAGConfig(
        ...
        embedding_model_name="sentence-transformers/all-MiniLM-L6-v2",  # Suitable embedding model
        working_dir="./hirag_cache_qwen3",
    ...
    # Load computer science contexts data
    cs_contexts_file = "/workspace/eval/datasets/cs/cs_unique_contexts.json"
    contexts = asyncio.run(load_cs_data(cs_contexts_file))
    
    # Insert data into knowledge graph with progress tracking
    asyncio.run(insert_data_with_progress(hirag_instance, contexts[:10]))  # Using first 10 for demo
    ...
    print("\nQwen3 HiRAG System execution completed!")
